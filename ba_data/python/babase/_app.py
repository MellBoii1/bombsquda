...
class App:
    ...
    def _update_state(self) -> None:
        ...
        # the shutdown process.
            ...
            if self.state is not AppState.SHUTTING_DOWN:
                ...
                lifecyclelog.info('app-state is now %s', self.state.name)
                import bauiv1
                if not _babase.app.config.get("squda_dontdomarioman", True):
                    ...
                    if isinstance(self.activity, MainMenuActivity):
                        ...
                        _babase.apptimer(3.4, lambda: self._on_shutting_down())
                    else:
    ...
    async def _wait_for_shutdown_suppressions(self) -> None:
        ...
        if duration > 1.0:
            logging.warning(
                'You wanna uh... go ahead and press the X button on me real quick? we took like... '
                '%.2f seconds to shutdown.',
                duration,
            )
