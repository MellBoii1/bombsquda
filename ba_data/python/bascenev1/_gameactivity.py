...
class GameActivity[PlayerT: bascenev1.Player, TeamT: bascenev1.Team](
    ...
    def _show_info(self) -> None:
        ...
        # if our activity doesn't tell us to suppress the activity's name, zoomtext it
        if not getattr(self, "suppress_zoomtext", False):
    ...
    def end_game(self, no_draw_sound: bool = False) -> None:
        if self.has_ended():
            return
        else:
            super().end_game()
    ...
    def _standard_time_limit_tick(self) -> None:
        ...
        # grab whatever music our gamemode plays
        # FIXME: since this plays gamemode music it doesn't know map specific music, 
        # and overrides the map's music
        # what is programming at this point man
        try:
            base_music = self.settings_raw.get("Music", self.default_music)
        except Exception:
