...
class CreditsActivity(bs.TeamGameActivity[bs.Player, bs.Team]):
    ...
    def on_begin(self) -> None:
        super().on_begin()
        self._start_credits()
        bs.timer(189.0, self.leave)
        self.showlogo('logo2')
        self.showlogo2('logo2')
    ...
    def showlogo2(self, tex: str | None = None):
        """
        Shows LOGOOO per request. idk im just copy pastin atp
        """
        texture = bs.gettexture(tex)
        ...
        bs.animate_array(self._logo_node, 'position', 2, {
            168.0: (0, -500),
            178.0: (0, 0) # scroll up slowly
        })
        
    def leave(self):
        """
        i don't know what tos ay
        """
        session = self.session
        bui.getsound('thankforplay').play()
        ba.apptimer(4.5, session.end) # this ends
