...
class MeteorShowerGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self.default_music = (
            bs.MusicType.EPICFAST if self._epic_mode else bs.MusicType.BUSINESS
        )
        if self._epic_mode:
    ...
    def on_begin(self) -> None:
        ...
        delay = 4.5 if len(self.players) > 2 else 1.5
        if self._epic_mode:
            delay *= 0.25
        ...
        bs.timer(10.0, self._check_end_game)
    ...
    def _decrement_meteor_time(self) -> None:
        # great we have to import fucking math. i hate math.
        import math
        ...
        # check if we haven't passed 50.4 seconds
        if elapsed < duration:
            # buncha math i dont understand xd
            t = elapsed / duration
            curve = math.pow(t, 4)
            
            self._meteor_time = start_delay + (end_delay - start_delay) * curve
        # otherwise...
        else:
            # Unleash hell if we've passed ~51 seconds.
            self._meteor_time = end_delay
    @override
