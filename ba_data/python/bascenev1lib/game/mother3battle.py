...
#
"""
use this to make mother 3-esque custom fights;
it ain't too accurate, but it looks pretty cool ngl!!!!
(i'm pretty sure there's gonna be complaints
about me trying to recreate another game in a random assert
physics game for no reason when i should be dev-ing
the ACTUAL physics game...)
also ummm please forgive me for the lack of comments..
i was a bit lazy... hopefully you know a bit about python
"""
...
class LoopingBackground:
    ...
    """
    def __init__(self, texture_name: str = 'scrollingBG', scroll_speed: float = 2.0):
...
class Mother3BattleSystem(bs.TeamGameActivity[Player, Team]):
    """like, mother 3 fight system or smth lamafao"""

    name = 'MOTHER 3 Battle System'
    description = 'a not-so-accurate replication of \nthe battle system from mother 3\nuse this to make your own ig...'
    ...
    def ues(self):
        ...
        if self.enemy_hp <= 0:
            # put whatever text you want your "enemy down" to be
            self.endtext = TypewriterText(
    ...
    def enemy_turn(self):
        ...
        if self.ended == True:
            return
        def enemy_attack():
            # if player is guarding, attack should be low
            if self.is_guarding == True:
                self.damagenum = random.randint(2,8)
            # otherwise, do a random number of damage here
            else:
                self.damagenum = random.randint(7,21)
    ...
    def _perform_attack(self):
        def actualattack():
            ...
            bs.getsound('enemybashed').play()
            # If it's our first time attacking, do damage
            # around 71-175 points.
            if self.timeschained == 0:
                self.damagenum = random.randint(71,175)
            # Otherwise if we're timing attacks, do small damage
            # of around 21-35 points 
            else:
    ...
    def do_ui(self):
        ...
        # Background.
        self.background = LoopingBackground('neoSpazIcon')
        # Top black border.
        ...
        # Background/the earthmeter itseeelf...
        ...
        # Player's name text on the EarthMeter.
        ...
        # Psychic Points on the EarthMeter.
        ...
        # Text for the option that's selected first.
        ...
        self.enemy = bs.newnode('image', 
            attrs={
                'texture': bs.gettexture('spazbound'),
                'absolute_scale': True,
                'position': (-10, 0),
                'attach': 'center',
                'opacity': 1.0,
